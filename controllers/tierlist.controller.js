...
export async function updateTierlist(req, res) {
  const { tierlistId } = req.params;
  const { title, likes, date } = req.body;
...
export async function getTierlistsByUserId(req, res) {
  ...
  try {
    ...
    console.log(e);
    res.status(500).json({
      message: `Error getting tierlists from user ${userId}`,
    });
  }
}

export async function getTierlistsByGameId(req, res){
    ...
    try {
        ...
        return res.status(500).json({
            message: `Error getting tierlists from game ${gameId}`,
        })
    }
}